...
class PredictionWorker(QThread):
    ...
    #     self.parse_server = Data_parse(self.MainWidget.parse_method, self.MainWidget.FileDir, self.MainWidget.savePath)
    #     self.parse_server.select_parse()
    #     try:
...
class Parse(QMainWindow):  # 主窗口类
    signal = pyqtSignal()

    def __init__(self):
        super(Parse, self).__init__()
        self.setWindowTitle("Data Parse Module")
    ...
    def init_LeftUI(self):
        ...
        # 设置左侧标签
        self.LeftLabel_1 = QLabel("医学数据解析:")
        self.LeftLabel_11 = QLabel("数据解析方法：")
        self.LeftLabel_2 = QLabel("解析结果:")
        ...
        self.DateType1 = QRadioButton("CT图像")
        # self.VideoType = QRadioButton("视频文件")
        self.DateType2 = QRadioButton("EM图像")
        self.DateType3 = QRadioButton("光学图像")
        self.DateType4 = QRadioButton("皮肤镜图像")
        ...
        self.LeftButton_4 = QPushButton(qtawesome.icon('fa.file', color='black'), "保存目录")
        ...
        self.Frame1Layout.addWidget(self.DateType4, 4, 3, 2, 1)
        self.Frame1Layout.addWidget(self.AddressShow, 5, 0, 2, 2)
        self.Frame1Layout.addWidget(self.LeftButton_1, 5, 2, 2, 2)
        self.Frame1Layout.addWidget(self.AddressShow1, 7, 0, 2, 2)
        self.Frame1Layout.addWidget(self.LeftButton_4, 7, 2, 2, 2)
        self.Frame1Layout.addWidget(self.LeftButton_2, 8, 2, 5, 2)
        self.Frame1Layout.addWidget(self.pbar, 8, 0, 5, 2)
    ...
    def predict(self):
        flag=0
        if self.group_parse.checkedButton():
            if self.PicType.isChecked() or self.DirType.isChecked():
                if not self.AddressShow.text()==" ":
                    if not self.AddressShow1.text()==" ":
                        flag=1
        if flag:
            id = self.group_parse.checkedId()
            print(id)
            if id==1:
                self.parse_method='CT'
            if id==2:
                self.parse_method='EM'
            if id==3:
                self.parse_method='Norm'
            if id==4:
                self.parse_method='ISIC'
            start = time.time()
            self.pbar.setValue(0)
            self.PredictionWorker.start()
            self.parse_server = Data_parse(self.parse_method, self.FileDir,
                                           self.savePath)
            self.parse_server.select_parse()
